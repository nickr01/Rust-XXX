...
impl RxPipeline {
    ...
    pub fn new(protocol: &'static types::Protocol, runtime: &'static types::Runtime) -> RxPipeline {
        ...
        let time_overlap = (runtime.target_input_sample_rate().0
            / (protocol.baud_rate().0 * runtime.rx_symbol_osr().0 as f32))
            as usize;
        dbg!(time_overlap);
    ...
    pub fn resample_context(&self, from_channels: usize, from_rate: u32) -> ResampleContext {
    ...
    pub fn write_mono_sample_buffer(
        ...
        // if audio_in.len() & !1 >= planned_load { // force even consumption
        {
            ...
            let samples_at_new_rate = if resample_context.from_rate
                == self.receiver.runtime.target_input_sample_rate().0 as u32
            {
                ...
                let input_adapter =
                    InterleavedSlice::new(mono_samples, 1, nbr_input_frames).unwrap();

                // create a buffer for the output
                let out_len = (mono_samples.len() as f64
                    * resample_context.resampler.resample_ratio())
                    as usize;
                // dbg!(mono_samples.len(), out_len);
                let mut outdata: Vec<f32> = vec![0f32; out_len];
                let outdata_capacity = outdata.len();
                let mut output_adapter =
                    InterleavedSlice::new_mut(&mut outdata, 1, outdata_capacity).unwrap();
                ...
                while input_frames_left >= input_frames_next {
                    let (frames_read, frames_written) = resample_context
                        .resampler
        ...
        let stale_time = types::Secs(
            (self.detector.wf.time_base().0 as f32
                / self.receiver.runtime.rx_symbol_osr().0 as f32)
