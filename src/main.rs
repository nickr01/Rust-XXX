...
use crate::rustxxx::{AudioSampleBuffer, Runtime};
...
fn do_audio_file_input(
    ...
    input_buff_writer: &mut rustxxx::ThreadedAudioWriter, 
    input_file: String,
...
fn main() -> Result<(), anyhow::Error> {
    ...
    let runtime: &'static rustxxx::Runtime = &rustxxx::TEST_FT8_RUNTIME;
    ...
    let mut receive_pipeline= pipeline::Pipeline::new(&rustxxx::TEST_PROTOCOL, runtime);
    ...
    let audio_input_buffer: AudioSampleBuffer = ringbuf::HeapRb::<f32>::new(rustxxx::AUDIO_INPUT_BUFSIZE);
    let audio_output_buffer: AudioSampleBuffer = ringbuf::HeapRb::<f32>::new(rustxxx::AUDIO_OUTPUT_BUFSIZE);

    let (mut audio_input_buff_writer, mut audio_input_buff_reader) = audio_input_buffer.split();
    let mut audio_input_buff_writer: rustxxx::ThreadedAudioWriter = std::sync::Arc::new(std::sync::Mutex::new(audio_input_buff_writer));
    let mut audio_input_buff_reader: rustxxx::ThreadedAudioReader = std::sync::Arc::new(std::sync::Mutex::new(audio_input_buff_reader));

    let (mut audio_output_buff_writer, mut audio_output_buff_reader) = audio_output_buffer.split();
    let mut audio_output_buff_writer: rustxxx::ThreadedAudioWriter = std::sync::Arc::new(std::sync::Mutex::new(audio_output_buff_writer));
    let mut audio_output_buff_reader: rustxxx::ThreadedAudioReader = std::sync::Arc::new(std::sync::Mutex::new(audio_output_buff_reader));
    ...
    let audio_input_stream = if let Some(audio_input_file_name) = opt.input_file {
        ...
        fn audio_input_data_callback(
            input: &[f32], 
            writer: &mut rustxxx::ThreadedAudioWriter,
        ) {
    ...
    let audio_output_stream = if let Some(_output_audio_file_name) = opt.output_file {
        ...
        fn audio_output_data_callback(output: &mut [f32], reader: &mut rustxxx::ThreadedAudioReader) {
