...
impl types::Modem {
    ...
    fn _sync_remove(
        ...
        for (i_tone, l0_tone) in l0_tones
            .iter()
            .enumerate()
            .take(self.protocol().total_symbols_nn().0)
        {
            if i_tone < 7 {
...
mod tests {
    ...
    fn test_layer1() {
        ...
        test_roundtrip(
            &mut modem,
            &[0u8; test_support::_TEST_PROTOCOL.nd().0].to_vec(),
        );
        test_roundtrip(
            &mut modem,
            &[5u8; test_support::_TEST_PROTOCOL.nd().0].to_vec(),
        );
