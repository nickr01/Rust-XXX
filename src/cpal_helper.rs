...
pub fn get_audio_input_device_by_id(
    host: &cpal::Host,
    ...
    match host.device_by_id(audio_input_device_id) {
        ...
        None => Err(anyhow::anyhow!(
            "Cannot get input device by id {}",
            audio_input_device_id
        )),
    }
}

pub fn get_audio_input_device_by_name(
    host: &cpal::Host,
    ...
    Err(anyhow::anyhow!(
        "Cannot get input device by name {}",
        audio_input_device_name
    ))
}

pub fn get_audio_input_device(
    host: &cpal::Host,
    audio_input_device_name: &String,
) -> Result<cpal::Device, anyhow::Error> {
    if audio_input_device_name.is_empty() {
        ...
        match get_audio_input_device_by_name(host, audio_input_device_name) {
            Ok(device) => Ok(device),
            Err(_) => get_audio_input_device_by_id(host, audio_input_device_name),
        }
    }
}

pub fn get_audio_input_device_default_config(
    host: &cpal::Host,
    audio_input_device_name: &String,
    ...
    if audio_input_device.supports_input() {
        ...
        dbg!();
        Err(anyhow::anyhow!(
            "Input device does not support input {}",
            audio_input_device_name
        ))
    }
}

pub fn get_audio_output_device_by_id(
    host: &cpal::Host,
    ...
    match host.device_by_id(audio_output_device_id) {
        Some(device) => Ok(device),
        None => Err(anyhow::anyhow!(
            "Cannot get output device by id {}",
            audio_output_device_id
        )),
    }
}

pub fn get_audio_output_device_by_name(
    host: &cpal::Host,
    audio_output_device_name: &String,
) -> Result<cpal::Device, anyhow::Error> {
    ...
    Err(anyhow::anyhow!(
        "Cannot get output device by name {}",
        audio_output_device_name
    ))
}

pub fn get_audio_output_device(
    host: &cpal::Host,
    audio_output_device_name: &String,
) -> Result<cpal::Device, anyhow::Error> {
    if audio_output_device_name.is_empty() {
        ...
        match get_audio_output_device_by_name(host, audio_output_device_name) {
            Ok(device) => Ok(device),
            Err(_) => get_audio_output_device_by_id(host, audio_output_device_name),
        }
    }
}

pub fn get_audio_output_device_default_config(
    host: &cpal::Host,
    audio_output_device_name: &String,
) -> Result<(cpal::Device, cpal::SupportedStreamConfig), anyhow::Error> {
    ...
    if audio_output_device.supports_output() {
        ...
        dbg!();
        Err(anyhow::anyhow!(
            "output device does not support output {}",
            audio_output_device_name
        ))
    }
}
...
mod tests {
    ...
    fn test_audio_output_device_by_name() {
        ...
        let _ = get_audio_output_device_by_name(&host, &name).expect("Cannot get device by name");
    }
    ...
    fn test_audio_output_device_by_id() {
        ...
        let _ = get_audio_output_device_by_id(&host, &id).expect("Cannot get device by id");
    }
