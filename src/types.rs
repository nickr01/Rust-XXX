...
impl Protocol {
    ...
    pub const fn symbol_period(&self) -> Secs {
        self.symbol_period
    }
    pub const fn baud_rate(&self) -> Hz {
        Hz(1.0 / self.symbol_period().0)
    }
    pub const fn _slot_time(&self) -> Secs {
        self.slot_time
    }
    pub const fn _slot_time_locked(&self) -> bool {
        self.slot_time_locked
    }
    pub const fn total_frame_time(&self) -> Secs {
        Secs(self.total_symbols_nn().0 as f32 * self.symbol_period().0)
    }
    pub const fn token_bits(&self) -> BitCount {
        self.token_bits
    }
    pub const fn token_tones(&self) -> RepeatCount {
        RepeatCount(1 << self.token_bits().0)
    }
    pub const fn nd(&self) -> SymbolCount {
        self.nd
    }
    pub const fn total_symbols_nn(&self) -> SymbolCount {
        self.total_symbols_nn
    } // Total channel symbols (FT8_NS + FT8_ND)
    pub const fn _ns(&self) -> SymbolCount {
        SymbolCount(self.length_sync.0 * self.num_sync.0)
    }
    pub const fn _length_sync(&self) -> SymbolCount {
        self.length_sync
    } // sync group length
    pub const fn num_sync(&self) -> RepeatCount {
        self.num_sync
    } // Number of sync groups
    pub const fn _length_ramp(&self) -> SymbolCount {
        self.length_ramp
    }
    pub const fn sync_offset(&self) -> SymbolCount {
        self.sync_offset
    } // Offset between sync groups
    pub const fn costas_pattern(&self) -> [u8; 7] {
        self.costas_pattern
    } //　Costas array
    pub const fn ldpc_n(&self) -> BitCount {
        self.ldpc_n
    } // Number of bits in the encoded message (payload with LDPC checksum bits)
    pub const fn ldpc_k(&self) -> BitCount {
        self.ldpc_k
    } // Number of payload bits (including CRC)
    pub const fn _ldpc_p(&self) -> BitCount {
        BitCount(self.ldpc_k().0 - self._crc_width().0)
    } // payload - no CRC
    pub const fn gray_map(&self) -> [u8; 8] {
        self.gray_map
    }
    pub const fn _gray_rmap(&self) -> [u8; 8] {
        self.gray_rmap
    }

    pub const fn _crc_pad_bits(&self) -> BitCount {
        BitCount(self.crc_params.prepad())
    }

    pub const fn _crc_polynomial(&self) -> BitMap {
        BitMap(self.crc_params.poly() as usize)
    }
    pub const fn _crc_width(&self) -> BitCount {
        BitCount(self.crc_params.width() as usize)
    }
    pub const fn _crc_start(&self) -> usize {
        self.crc_params.init() as usize
    }
    pub const fn _crc_xor(&self) -> usize {
        self.crc_params.xorout() as usize
    }

    pub const fn _symbol_bt(&self) -> f32 {
        self.symbol_bt
    }

    // pub const fn ramp_symbols(&self) -> SymbolCount { self.ramp_symbols }

    pub const fn ldpc_m(&self) -> BitCount {
        BitCount(self.ldpc_n().0 - self.ldpc_k().0)
    }

    pub const fn ldpc_n_bytes(&self) -> ByteCount {
        bits2bytes(self.ldpc_n())
    } // Number of whole bytes needed to store 174 bits (full message)
    pub const fn _ldpc_k_bytes(&self) -> ByteCount {
        bits2bytes(self.ldpc_k())
    } // Number of whole bytes needed to store 91 bits (payload + CRC only)
    pub const fn _ldpc_p_bytes(&self) -> ByteCount {
        bits2bytes(self._ldpc_p())
    } // Number of whole bytes needed store payload only
    pub const fn _ldpc_p_padded_bytes(&self) -> ByteCount {
        bits2bytes(BitCount(self._ldpc_p().0 + self._crc_pad_bits().0))
    }
...
impl Runtime {
    ...
    pub fn window_function(&self, i: usize, n: usize) -> f32 {
        ...
        match self.window_function {
            ...
            WindowFunction::_Hann => (std::f32::consts::PI * i as f32 / n as f32).sin().powi(2),
...
impl Modem {
    pub fn new(protocol: &'static Protocol, runtime: &'static Runtime) -> Modem {
...
impl Message {
    pub fn new(time_secs: Secs, freq_hz: Hz, c_score: f32, codeword: CodeWord) -> Message {
        Message {
            time_secs,
            freq_hz,
            c_score,
    ...
    pub fn from_vec(cwv: Vec<u8>) -> Result<CodeWord, error::XxxError> {
        if cwv.is_empty() {
            ...
            //     codeword += b as u128;
            // }
            let cwv = cwv[0..10].to_vec().into_iter().collect();
            Ok(CodeWord(cwv))
        }
    }
}
